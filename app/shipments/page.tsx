...
export default function ShipmentsPage() {
  ...
  const { data: shipmentsResponse, isLoading, error } = useGetMyShipmentsQuery({ page: 1, itemsPerPage: 1000 })
  ...
  const filteredShipments = shipments.filter((shipment) => {
    // Filter by tab
    if (activeTab === "active" && shipment.status === "delivered") return false
    if (activeTab === "delivered" && shipment.status !== "delivered") return false
    if (activeTab === "processing" && shipment.status !== "processing" && shipment.status !== "ready") return false
