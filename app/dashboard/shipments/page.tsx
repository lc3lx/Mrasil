...
import { Package, Truck, MapPin, Clock, Search, Edit, ArrowLeft, TrendingUp, Calendar, Eye, Trash2, Filter, Plus, CheckCircle } from 'lucide-react';
    ...
    <DashboardLayout>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50 to-blue-100">
        ...
        >
          {/* Header */}
          <motion.div 
            className="relative overflow-hidden bg-gradient-to-br from-cyan-600 via-blue-700 to-indigo-800 rounded-2xl shadow-2xl"
          ...
          >
            ...
            <div className="relative p-8">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-4xl font-bold text-white mb-2">إدارة الشحنات</h1>
                  <p className="text-cyan-100 text-lg">تتبع ومتابعة جميع الشحنات والطرود</p>
                </div>
            ...
            {isLoading ? (
              ...
              <>
                ...
                >
                  ...
                  <div className="relative flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-4xl font-bold text-gray-900 mb-3">{stats?.data?.shipments?.total?.toLocaleString() || '0'}</p>
                      <div className="flex items-center">
                        <TrendingUp className="w-4 h-4 text-green-500 ml-1" />
                        <span className="text-sm text-green-600 font-medium">+18% من الشهر الماضي</span>
                      </div>
                    </div>
                ...
                >
                  ...
                  <div className="relative flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-600 mb-2">جاهزة للاستلام</p>
                      <p className="text-4xl font-bold text-gray-900 mb-3">{stats?.data?.shipments?.pending?.toLocaleString() || '0'}</p>
                      <div className="flex items-center">
                        <Clock className="w-4 h-4 text-amber-500 ml-1" />
                        <span className="text-sm text-amber-600 font-medium">في انتظار الاستلام</span>
                      </div>
                    </div>
                ...
                >
                  ...
                  <div className="relative flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-600 mb-2">في الطريق</p>
                      <p className="text-4xl font-bold text-gray-900 mb-3">{stats?.data?.shipments?.inTransit?.toLocaleString() || '0'}</p>
                      <div className="flex items-center">
                        <TrendingUp className="w-4 h-4 text-green-500 ml-1" />
                ...
                >
                  ...
                  <div className="relative flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-4xl font-bold text-gray-900 mb-3">{stats?.data?.shipments?.delivered?.toLocaleString() || '0'}</p>
                      <div className="flex items-center">
                        <TrendingUp className="w-4 h-4 text-green-500 ml-1" />
                        <span className="text-sm text-green-600 font-medium">+25% من الشهر الماضي</span>
                      </div>
                    </div>
          ...
          >
            ...
            <div className="absolute top-0 right-0 w-40 h-40 bg-cyan-500/5 rounded-full -translate-y-20 translate-x-20"></div>
            ...
            <div className="relative space-y-6">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-gray-900">قائمة الشحنات</h3>
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-4 pr-12 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent outline-none transition-all w-full sm:w-64"
                    />
                  </div>
                  ...
                  <select
                    ...
                    className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent outline-none transition-all"
                  >
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200">
                <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
                  <div className="grid grid-cols-1 md:grid-cols-7 gap-4 text-sm font-semibold text-gray-700">
                    <div>رقم التتبع</div>
                    <div className="hidden md:block">العميل</div>
                    <div className="hidden md:block">شركة الشحن</div>
                    <div className="hidden md:block">الحالة</div>
                    <div className="hidden md:block">قيمة الطلب</div>
                ...
                <div className="divide-y divide-gray-200">
                  {listLoading ? (
                    Array.from({ length: 5 }).map((_, index) => (
                      <div key={index} className="px-6 py-4 animate-pulse">
                        <div className="grid grid-cols-1 md:grid-cols-7 gap-4 items-center">
                          ...
                          <div className="h-4 bg-gray-300 rounded w-28 hidden md:block"></div>
                          <div className="h-8 bg-gray-300 rounded w-32 hidden md:block"></div>
                        </div>
                      </div>
                    ))
                  ) : shipmentsResponse?.data?.length > 0 ? (
                    shipmentsResponse.data.map((shipment: any, index: number) => (
                      <motion.div
                        key={shipment._id}
                      ...
                      >
                        <div className="grid grid-cols-1 md:grid-cols-7 gap-4 items-center">
                          <div className="flex items-center space-x-3 space-x-reverse">
                            ...
                            <div>
                              <div className="font-medium text-gray-900">{shipment.trackingId || shipment.companyshipmentid || `#${index + 1}`}</div>
                              <div className="text-sm text-gray-500">شحنة #{index + 1}</div>
                            </div>
                          </div>
                          
                          <div className="hidden md:block text-gray-600">
                            {`${shipment.customerId?.firstName || ''} ${shipment.customerId?.lastName || ''}`.trim() || 'غير محدد'}
                          </div>
                          
                          <div className="hidden md:block text-gray-600">
                            {shipment.shapmentCompany || 'غير محدد'}
                          </div>
                          
                          <div className="hidden md:block">
                            <StatusBadge status={shipment.shipmentstates} type="shipment" />
                          </div>
                          
                          <div className="hidden md:block text-gray-600 font-medium">
                            {(shipment.ordervalue ?? shipment.totalprice ?? 0).toLocaleString()} ريال
                          </div>
                          ...
                          <div className="hidden md:block">
                            <select
                              defaultValue={shipment.shipmentstates}
                              disabled={updatingStatus}
                              onChange={async (e) => {
                                try {
                                  await updateShipmentStatus({ shipmentId: shipment._id, status: e.target.value }).unwrap();
                                  await refetch();
                              ...
                              className="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white hover:bg-gray-50 focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200"
                            >
                              <option value="IN_TRANSIT">في الطريق</option>
                              <option value="READY_FOR_PICKUP">جاهزة للاستلام</option>
                              <option value="Delivered">تم التسليم</option>
                              <option value="Canceled" disabled={shipment.shipmentstates !== "READY_FOR_PICKUP"}>ملغية</option>
                            </select>
                          </div>
                        </div>
                      </motion.div>
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-500 mb-6">لم يتم العثور على أي شحنات بالمعايير المحددة</p>
                    </div>
                  )}
                </div>
              </div>
