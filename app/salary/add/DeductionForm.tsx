...
import { useAddDeductionMutation } from "../../api/salaryApi";
import { useGetAllTeamMembersQuery } from "../../api/teamApi";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";

export default function DeductionForm({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await addDeduction({ id: employeeId, amount: Number(amount), reason }).unwrap();
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>خصم من مرتب موظف</DialogTitle>
        </DialogHeader>
