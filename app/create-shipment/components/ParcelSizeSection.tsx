...
export function ParcelSizeSection({
  parcelsData,
  setValue,
  errors,
}: ParcelSizeSectionProps) {
  // Map API data to card format with unique key
  const sizeCards: SizeCard[] = (parcelsData?.data || []).map(
  ...
  const handleCustomParcelFormSubmit = async (
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              ...
              <div className="flex flex-col items-center text-center gap-2 relative">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    سم {card.dims.length} × {card.dims.width} × {card.dims.high}
                  </div>
                  {card.maxWeight && (
        ...
        <Button
          type="button"
          className={` v7-neu-card-inner  p-4 cursor-pointer  hover:bg-[#f0f4f8] h-[10rem] w-[18rem]  flex flex-col  gap-2`}
      ...
      {selectedSize &&
        (() => {
          const selectedCard = sizeCards.find(
            (c: SizeCard) => c.key === selectedSize
          );
      ...
      <Dialog open={customModalOpen} onOpenChange={setCustomModalOpen}>
        <DialogContent className=" flex  flex-col gap-8  sm:h-[16rem] w-[40rem]">
          ...
          <form
            onSubmit={handleCustomParcelFormSubmit}
            className="space-y-2  grid grid-cols-3 gap-4 "
          >
            <Input
            ...
            <DialogFooter className="   ">
              <Button
                type="submit"
                className="bg-blue-500 text-white  w-[20rem]"
