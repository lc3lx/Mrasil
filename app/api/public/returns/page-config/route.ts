...
/**
 * API عامة لجلب تخصيص صفحة الاسترجاع للتاجر.
 * - إذا token يشبه الرابط الفريد (slug): استدعاء الباكند GET /customer/return-page/:slug (بدون تسجيل دخول).
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isSlugLike(cleanToken)) {
      const res = await fetch(`${API_BASE_URL}/customer/return-page/${encodeURIComponent(cleanToken)}`, {
        method: "GET",
      ...
      if (!res.ok) {
        return NextResponse.json(
          { success: false, message: data?.message || "صفحة الاسترجاع غير موجودة" },
          { status: res.status }
        );
      }
    ...
    const settings = data?.data?.returnPageSettings ?? null;
    return NextResponse.json({ success: true, data: settings });
  } catch (e) {
    console.error("[public/returns/page-config]", e);
    return NextResponse.json(
